...
const LspConfig = lsp_config.LspConfig;
const TypescriptMode = lsp_config.TypescriptMode;
const LspAdapter = lsp_config.LspAdapter;
...
pub const Client = struct {
    ...
    pub fn startForFile(self: *Client, file_path: []const u8, config: *const LspConfig) !void {
        self.stop();
    ...
    fn collectStartCandidates(
        ...
        if (is_typescript) {
            if (config.typescript_command) |command| {
                ...
                if (config.typescript_mode != .tsls) {
                    try candidates.append(.{
    ...
    fn allowTypescriptPreset(config: *const LspConfig, preset_name: []const u8) bool {
        return switch (config.typescript_mode) {
...
test "typescript preset allowlist includes npx tsls fallback" {
    const allocator = std.testing.allocator;
    const auto_cfg = lsp_config.LspConfig{
        .enable_lsp = true,
    ...
    const tsls_cfg = lsp_config.LspConfig{
        .enable_lsp = true,
        .typescript_mode = .tsls,
        .typescript_command = null,
        .typescript_args = &.{},
        .typescript_root_markers = &.{},
        .zig_enabled = null,
